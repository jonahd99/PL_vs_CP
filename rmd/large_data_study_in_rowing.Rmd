...
```{r}
if (rewrangle_data) {
  read_excel(here("data", "seiler_row_ski_ergometer.xlsx"), sheet = 2) %>%
    clean_names() %>%
    mutate(id = 1:n()) %>%
    select(!athlete) %>%
  ...
  write_rds(df_seiler, file = here("data", "df_seiler.Rds"))
} else {
  df_seiler <- read_rds(here("data", "df_seiler.Rds"))
}
```
...
if (rewrangle_data) {
  ...
  write_rds(df_nonathlon, file = here("data", "df_nonathlon.Rds"))
} else {
  df_nonathlon <- read_rds(here("data", "df_nonathlon.Rds"))
}
```
...
if (recompute_errors) {
  ...
  write_rds(df_errors_seiler, file = here("data", "df_errors_seiler.Rds"))
  write_rds(df_errors_nonathlon, file = here("data", "df_errors_nonathlon.Rds"))
...
# plot_errors(
#   data = df_errors_both,
#   fig_name = "errors_rowing_both.tex",
#   width = 5,
#   show.legend = TRUE,
