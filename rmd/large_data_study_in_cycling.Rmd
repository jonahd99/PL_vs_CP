...
if (rewrangle_data) {
  ...
  gc_file_path <- here("figure_data","gc_data")
  ...
  write_rds(df_gc, file = here("figure_data", "df_gc.Rds"))
  
} else{
  df_gc <- read_rds(here("figure_data", "df_gc.Rds"))
}
...
if (recompute_errors) {
  ...
  write_rds(df_errors_gc, file = here("figure_data", "df_errors_gc.Rds"))
