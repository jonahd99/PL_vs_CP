################################################################################
# Important note: This script is not called in any rmd file on github          #
################################################################################
...
fit_Omni3cp <- function(data_frame,startCP,StartW){
  ...
  df_fit <- data.frame(x = 1:7200)
  omni_3cp_pd_fit <- predict(mod, newdata = df_fit)
  
  return(omni_3cp_pd_fit)
...
fit_OmExp <- function(data_frame,startCP,StartW){
  ...
  df_fit <- data.frame(x = 1:7200)
  omni_exp_pd_fit <- predict(mod, newdata = df_fit)
  
  return(omni_exp_pd_fit)
